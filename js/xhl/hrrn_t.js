...
function insert(st, nt) {
    ...
    for (var j = 0; j < 9; j++) {
        ...
        else if (j == 5) td.innerHTML = parr.key_rio[lon];
        else if (j == 6) {
        ...
        } else if (j == 7) td.innerHTML = parr.key_ztime[lon];
        else if (j == 8) td.innerHTML = parr.key_dqztime[lon];
        else if (j == 3) { //还需时间
            td.innerHTML = parr.key_rtime[lon];
        } else if (j == 4) { //等待时间
            td.innerHTML = parr.key_waittime[lon];
        }
        tr.appendChild(td);
    }
}
