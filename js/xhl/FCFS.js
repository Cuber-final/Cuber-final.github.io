...
function progress_start(num, start_time) { //某个进程开始运行
    ...
    em.innerHTML = '开始: P' + num + ' ** 开始时间：' + start_time + 's ** 结束时间：' + ik + 's';
...
//一个进程结束时挑选等待时间最高的进程为其分配CPU,返回进程的序号
function celect(second) {
    ...
    for (var i = child.length - 1; i >= 0; i--) {
        if (parr.key_waittime[i] > minn && parr.key_statue[i] == 0 && parr.key_stime[i] <= second) {
            num = i;
            minn = parr.key_waittime[i];
        }
    }
    return num;
}
...
function updata(second) {
    ...
    for (var i = child.length - 1; i >= 0; i--) {
        ...
        if (parr.key_stime[i] <= second && parr.key_statue[i] == 0) {
            ...
            td[3].innerHTML = parr.key_waittime[i];
...
function insert(st, nt) {
    ...
    for (var j = 0; j < 7; j++) {
        ...
        if (j == 0)
        ...
        else if (j == 3) td.innerHTML = parr.key_waittime[lon];
        else if (j == 4) {
            ...
            td.innerHTML = parr.key_ztime[lon];
        } else td.innerHTML = parr.key_dqztime[lon];
        tr.appendChild(td);
    }
}
...
btn_learn.onclick = function() { //开始启动计时
    alert("  FCFS主要从“公平”的角度考虑（类似于我们生活中排队买东西的例子）,按照作业/进程到达的先后顺序进行服务,是非抢占的");
}